...
class StackFarthestPointSampling(Function):
    ...
    def forward(ctx, xyz, xyz_batch_cnt, npoint):
        ...
        if not isinstance(npoint, torch.Tensor):  # fixed number of sample
            if not isinstance(npoint, list):
                npoint = [npoint for i in range(batch_size)]
            else:
                assert len(npoint) == xyz_batch_cnt.size()[0]
            npoint = torch.tensor(npoint, device=xyz.device).int()
        else:   # different number of samples for each batch
            if npoint.size()[0] == 1:
                npoint = npoint.expand([xyz_batch_cnt.size()[0], ]).contiguous()
            else: assert  npoint.size()[0] == xyz_batch_cnt.size()[0]
...
if __name__ == '__main__':
    ...
    # points = torch.randint(0, 500, (100, 3)).cuda()
    # batch_idx = torch.tensor([0, 100, 200, 300, 499])
    #
    # sampling = StackFarthestPointSampling()
    # output, npoint = sampling(points, batch_idx, 50)
